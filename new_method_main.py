...
files_list = [['file name','commit massage','commit hash','author name','committer date',
              ...
              'complexity', 'nloc','java file before commit','java file after commit']]
...
for my_url in list_of_repo_urls:
    for commit in Repository(my_url, only_modifications_with_file_types=['.java'],
                             to=dt, only_in_branch='main').traverse_commits():
        for file in commit.modified_files:
            if file.source_code and file.source_code_before and file.filename.endswith(".java"):
                ...
                list_of_java_files.append(os.path.join(java_files_directory, 'before_' + file_name))
                list_of_java_files.append(os.path.join(java_files_directory, 'current_' + file_name))
                java_files_for_tool.append(list_of_java_files)
                row_of_csv.append(os.path.join(java_files_directory, 'before_' + file_name))
                row_of_csv.append(os.path.join(java_files_directory, 'current_' + file_name))
...
csv_java_files = [['java file before commit', 'java file after commit', 'java file readability before commit'
                   , 'java file readability after commit', 'difference']]

print(java_files_for_tool)
for jf in java_files_for_tool:
    row_in_csv =[]
    row_in_csv.append(jf[0])
    row_in_csv.append(jf[1])
    val1 = get_readability_score(jf[0])
    row_in_csv.append(val1)
    val2 = get_readability_score(jf[1])
    row_in_csv.append(val2)
    try:
         diff = val2 - val1
    except ValueError:
         diff = 'not available'
    row_in_csv.append(diff)
    csv_java_files.append(row_in_csv)



with open('readability.csv', 'w', newline='') as f:
     writer = csv.writer(f)
     for row in csv_java_files:
         writer.writerow(row)